...
const DailyVoiceRecorder = ({ user }) => {
    ...
    const [confirmLangChange, setConfirmLangChange] = useState(null);
    ...
    const loadEntries = useCallback(async (loadMore = false) => {
        ...
        try {
            ...
            setEntries(prev =>
                loadMore ? [...prev, ...newEntries] : newEntries
            );
    ...
    const handleLanguageChange = (newLanguage) => {
        ...
        if (finalTranscript.trim() || interimTranscript.trim()) {
            setConfirmLangChange(newLanguage);
            return;
        }
    ...
    const saveEdit = async (id) => {
        try {
            ...
            setEntries(prev =>
                prev.map(entry =>
                    entry.id === id
                        ? { ...entry, title: editedTitle, text: editedText }
                        : entry
                )
            );
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-purple-50 via-pink-50 to-indigo-100">
                ...
                <div className="bg-white/80 backdrop-blur-sm rounded-3xl p-8 shadow-xl border border-white/20">
                    {savedFeedback && (
                        <div className="flex justify-center mt-4">
                            <div className="px-4 py-2 bg-green-100 text-green-700 rounded-full text-sm font-medium shadow-sm animate-fade-in">
                                Saved
                            </div>
                        </div>
                    )}
                ...
                {entries.length > 0 && (
                    <div className="space-y-6">
                                    ...
                                    {editingId === entry.id ? (
                                        <>
                                            <input
                                                ...
                                                className="w-full mb-3 px-3 py-2 border border-purple-200 rounded-lg focus:ring-2 focus:ring-purple-400"
                                            />
                                        ...
                                        <div className="mt-4 border-t border-gray-100 pt-3">
                                            <div className="flex flex-wrap items-center gap-2">
                                                ...
                                                <select
                                                    ...
                                                    onChange={(e) =>
                                                        setTranslateTo({ ...translateTo, [entry.id]: e.target.value })
                                                    }
                                                ...
                                                <button
                                                    onClick={() =>
                                                        translateEntry(entry.id, translateTo[entry.id])
                                                    }
                                                    ...
                                                    <p className="text-sm font-medium text-blue-800 mb-1">
                                                        Translation (
                                                        {
                                                            languageOptions.find(
                                                                (opt) =>
                                                                    opt.code === translatedEntries[entry.id].lang
                                                            )?.name
                                                        }
                                                        ):
                                                    </p>
                                                    <p className="text-gray-700">
                                                        {translatedEntries[entry.id].text}
                                                    </p>
                                                </div>
                                            )}
                                        </div>
                        ...
                        <div className="flex justify-center gap-4 mt-6">
                            {hasMore && !loadingMore && !showAll && (
                                <button
                                    onClick={() => loadEntries(true)}
                                    className="px-6 py-2 bg-purple-100 text-purple-700 rounded-full hover:bg-purple-200 transition"
                                >
                                    Load more
                                </button>
                            )}
            ...
            <ConfirmModal
                open={!!confirmLangChange}
                title="Unsaved changes"
                message="You have unsaved content. Changing language will discard it."
                confirmText="Change language"
                cancelText="Cancel"
                onCancel={() => setConfirmLangChange(null)}
                onConfirm={() => {
                    setFinalTranscript('');
                    setInterimTranscript('');
                    setLanguage(confirmLangChange);
                    setConfirmLangChange(null);
                }}
            />
        </div>
    );
